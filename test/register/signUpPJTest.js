...
import { userDataPJ } from '../../data/form.data.js'
import { assert } from 'chai'

describe('My register application', async () => {
    it('should register a new user PJ', async () => {
        await (await loginPage).open()
        await (await homePage).clickBtnEnter()
        await formPage.registerPJUser(userDataPJ)
        await assert.equal(await accountHomePage.getAccountName(), userDataPJ.companyName, `O nome do usuário não foi exibido.`)
